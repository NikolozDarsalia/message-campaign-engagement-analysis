...
def create_rolling_features(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    df["is_email"] = (
        (df["channel_x"] == "email").astype(int) if "channel_x" in df.columns else 0
    )
    df["is_push"] = (
        (df["channel_x"] == "mobile_push").astype(int)
    ...
    for col, prefix, agg_func, days, period in rolling_configs:
        ...
        try:
            rolling_obj = df_indexed.groupby("client_id")[col].rolling(
                days, closed="left"
            )
