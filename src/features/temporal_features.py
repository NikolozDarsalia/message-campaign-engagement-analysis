...
def create_temporal_features(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    df["days_since_last_msg"] = (
        df.groupby("client_id")["sent_at"].diff().dt.total_seconds().div(86400)
    )
    ...
    if verbose:
        print(
            f"  ✅ Created {14 + (2 if 'channel_x' in df.columns else 0)} temporal features"
        )
