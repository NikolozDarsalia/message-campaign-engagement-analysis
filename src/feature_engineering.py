...
from features.global_campaign_performance_features import (
    add_global_campaign_performance_features,
)
from features.clients_expectation_deviation_features import (
    add_client_vs_global_engagement_gap,
)
...
def engineer_all_features(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    if verbose:
        ...
        print(
            f"Total features created: {df.shape[1] - len(df.select_dtypes(include='object').columns)}"
        )
        print("=" * 60 + "\n")

    df.to_parquet("../sampling/data/whole_df_with_fe.parquet")
